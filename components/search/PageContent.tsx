...
export default function SearchPageContent(): JSX.Element {
  const items = React.useMemo(() => (
    Array.from({ length: 6 }).map((_, i) => ({
      ...
      hours: "6am - 10pm",
      pricing: [
        { size: "S (6' x 8')", originalPrice: "7200", currentPrice: "5000.40" },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                ...
                <div className="h-96 lg:h-[600px] overflow-auto pr-2">
                  <div className="space-y-4">
                    {items.map((it, idx) => (
                      <LocationCard key={idx} name={it.name} address={it.address} hours={it.hours} pricing={it.pricing} imageUrl={it.imageUrl} />
                    ))}
                  </div>
                </div>
              </div>
